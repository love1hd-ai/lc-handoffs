...
class HandoffsAgent:
    ...
    def extract_content(self, state: HandoffsState) -> HandoffsState:
        ...
        if extraction_prompt is None:
            ...
            extraction_prompt_template = """请从以下文本中提取消费或收入信息, 包括:
1. 时间, 格式: yyyy-mm-dd hh:mm:ss(如果文本中没有明确时间, 使用当前时间)
2. 内容/项目(如果文本中有地点信息, 格式为"项目(地点)", 如"在大丰买衣服"->"衣服(大丰)"; 如果原文本已包含括号, 保持原格式)
3. 类型/分类(格式: 具体分类-动作, 如"服装-购买"、"餐饮-外卖"、"收入"等)
4. 金额(支持中文数字, 如"四块钱"转换为4.00, "一百元"转换为100.00, "四十五"转换为45.00)
...
- 金额提取:
  * 支持阿拉伯数字: 如"花了100元"、"4块钱"、"¥50"、"收入100元"
  * 支持中文数字: 如"四块钱"->4.00, "一百元"->100.00, "五十块"->50.00, "四十五"->45.00
  * 金额符号规则:
    - 如果是收入(如"收入"、"收到"、"工资"、"奖金"等), 金额为正数, 例如: 100.00
    - 如果是消费/支出(如"花了"、"买了"、"消费"、"支付"等), 金额为负数, 例如: -100.00

重要: 请积极提取信息, 即使文本比较口语化或简短. 只要文本中包含消费或收入相关的信息(如"花了"、"买了"、"消费"、"收入"、"收到"、"工资"、"奖金"等关键词), 就应该提取出来. 注意区分收入和支出, 收入金额为正数, 支出金额为负数.
